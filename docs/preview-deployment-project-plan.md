...
**Key insight:** Deploy service must dogfood itself before we remove old deployment methods from other apps.

**Flow:**
1. Keep existing deploy scripts while building deploy service
2. Build and test deploy service using old deploy methods
3. Bootstrap deploy service manually
4. Add GitHub Actions to deploy service - make it self-deploying
5. Test deploy service can update itself
6. Only then add GitHub Actions to other apps
7. Finally remove legacy deploy scripts once everything uses new system

---

# Epic 1: Foundation - User Systemd Migration

**Goal:** Eliminate sudo requirements for app management by migrating to user systemd services. Keep existing deploy scripts for now.
...
**Description:**
Migrate doughughes-net app to user systemd service. Keep existing deploy.sh scripts working.
...
**Description:**
Migrate remaining apps to user systemd services. Keep existing deploy.sh scripts working.
...
- Update `deploy.sh` to use `systemctl --user` (will be removed later)
...
# Epic 2: Simplification - Remove Auth & Prepare for Generated Configs
...
- Remove users.json and sessions.json from .gitignore
...
- Remove "v2" badge (no longer needed)
- Test locally
...
- Deploy changes using existing deploy scripts
...
**Description:**
Remove caddy.conf and caddy-subdomain.conf from all app repos since deploy service will generate them dynamically. Keep existing Caddy configs on server working for now.
...
- random-word
- lottery-numbers
- doughughes-net (will get special catch-all generated by deploy service)
- app-template
...
- Commit changes
- Note: Don't delete configs on server yet - they're still active
...
- Server configs still in place (apps still working)
...
- For each app:
  ...
  - Deploy using existing deploy scripts
...
**Tasks:**
- Create `~/preview-deployments/` directory on server
- Create `~/preview-deployments/ports.json` with current allocations:
  ```json
  {
    "doughughes-net": 8000,
    "random-word": 8001,
...
**Goal:** Build the deploy service API while old deployment methods still work.
...
**Description:**
Set up deploy-service app with basic structure using app-template.

**Tasks:**
- Clone app-template to new deploy-service repo
...
- Keep deploy.sh for now (will use to bootstrap)
- Test locally
...
- Has deploy.sh script for manual deployment
- Ready to be deployed old-fashioned way
...
- Check if deployment exists in `~/apps/{deployment_id}`:
  - If exists: git pull to update
  - If not: git clone to `~/apps/{deployment_id}`
...
- Generate systemd service file in `~/.config/systemd/user/`:
  ...
  - ExecStart: determined from app (Python, Go, Node, etc.)
- Generate `~/.config/caddy/{deployment_id}.caddy-subdomain.conf`:
  - Subdomain: calculated above
  - Reverse proxy to localhost:{port}
- Special case for doughughes-net production: also generate `~/.config/caddy/doughughes-net.caddy.conf`:
  ```
  handle {
...
- Redeploying existing deployment reuses same port and updates code
- New deployments get next available port
- Service is running and accessible
- Returns correct URL
- Handles errors gracefully (cleanup on failure)
- Works for brand new apps (zero manual setup)
- doughughes-net gets both subdomain and catch-all configs when deployed to production
...
- Remove service file from `~/.config/systemd/user/{deployment_id}.service`
...
- Deploy when many ports are allocated
...
- Deploy brand new app that doesn't exist yet
- Redeploy existing app (verify port reuse and git pull)
...
- Add API documentation (endpoints, authentication, examples)
...
# Epic 5: Bootstrap Deploy Service

**Goal:** Manually deploy deploy-service to production using old-fashioned deploy.sh method.
...
**Description:**
Manually deploy the deploy-service app to the server using existing deploy.sh script since it can't deploy itself initially.

**Tasks:**
- Push deploy-service repo to GitHub
- SSH to server
- Clone deploy-service repo to `~/apps/deploy-service`
- Manually allocate port (next available, likely 8006)
- Update ports.json: `"deploy-service": 8006`
- Create systemd service manually in `~/.config/systemd/user/deploy-service.service`
  - WorkingDirectory: ~/apps/deploy-service
  - Environment: PORT=8006
- Create Caddy config manually in `~/.config/caddy/deploy-service.caddy-subdomain.conf`
  - Subdomain: deploy.doughughes.net
  - Reverse proxy to localhost:8006
...
- Test API endpoints work (POST test deployment)
- Generate and save DEPLOY_API_KEY
- Store API key securely for GitHub Actions

**Acceptance Criteria:**
- Deploy service running and accessible at deploy.doughughes.net
- API endpoints respond correctly
- API key authentication works
- Can successfully deploy a test preview
- Ready to have GitHub Actions added

**Dependencies:** Deploy Service Testing & Hardening

---

# Epic 6: Make Deploy Service Self-Deploying

**Goal:** Add GitHub Actions to deploy-service so it can deploy itself.

**Estimated Time:** ~4.5 hours

---

## Create GitHub Workflows for Deploy Service
**Priority:** P0
**Estimate:** 2 hours

**Description:**
Add GitHub Actions workflows to deploy-service repo so it can deploy itself via the API it provides.

**Tasks:**
- Create `.github/workflows/preview-deploy.yml`:
  - On PR: deploy preview to pr-{number}.deploy-service.doughughes.net
  - Test the deploy service works for itself
- Create `.github/workflows/deploy-production.yml`:
  - On push to main: deploy to deploy.doughughes.net (pr_number=null)
- Create `.github/workflows/cleanup-preview.yml`:
  - On PR close: cleanup preview
- Add DEPLOY_API_KEY to deploy-service GitHub repo secrets
- Test workflows with a test PR

**Acceptance Criteria:**
- Workflows trigger correctly
- Deploy service can deploy itself
- Preview deployments of deploy service work
- Production deployments update the live deploy service
- Validates the deploy service works correctly

**Dependencies:** Bootstrap Deploy Service to Production

---

## Test Deploy Service Self-Deployment
**Priority:** P0
**Estimate:** 2 hours

**Description:**
Validate that deploy service can successfully update itself via GitHub Actions. This is critical before rolling out to other apps.

**Tasks:**
- Create test PR in deploy-service repo with a small change
- Verify preview deploys to pr-X.deploy-service.doughughes.net
- Test preview works correctly
- Merge PR
- Verify production updates at deploy.doughughes.net
- Verify preview cleanup happens
- Test production can still deploy other apps
- Document any issues or edge cases discovered

**Acceptance Criteria:**
- Deploy service successfully deploys preview of itself
- Deploy service successfully updates its production deployment
- Cleanup works correctly
- No downtime during production update
- Deploy service remains functional after self-update
- Ready to roll out to other apps

**Dependencies:** Create GitHub Workflows for Deploy Service

---

## Update Infrastructure Caddyfile for Generated Configs
**Priority:** P0
**Estimate:** 30 minutes

**Description:**
Update the infrastructure Caddyfile to import configs from `~/.config/caddy/` where deploy service generates them, instead of from `~/apps/` where they currently live.

**Tasks:**
- Update `~/infrastructure/caddy/Caddyfile`:
  ```
  # Import all subdomain configs (generated by deploy service)
  import /home/dhughes/.config/caddy/*.caddy-subdomain.conf

  # Root domain (doughughes-net provides catch-all)
  doughughes.net, www.doughughes.net {
      import /home/dhughes/.config/caddy/*.caddy.conf
  }
  ```
- Deploy updated Caddyfile: `sudo ~/infrastructure/deploy.sh caddy`
- Verify all apps still accessible
- Verify doughughes-net still serves root domain

**Acceptance Criteria:**
- Caddyfile imports from ~/.config/caddy/
- All existing apps still work (old configs in place)
- Ready for deploy service to generate new configs
- No downtime during change

**Dependencies:** Test Deploy Service Self-Deployment

---

# Epic 7: GitHub Actions Integration

**Goal:** Create workflow templates and add to app-template.
...
**Tasks:**
- Create workflow file based on deploy-service's working workflow
...
**Acceptance Criteria:**
- Workflow file complete and tested
- Extracts app name correctly
- Posts preview URL to PR
- Easy to customize for different test frameworks

**Dependencies:** Update Infrastructure Caddyfile for Generated Configs
...
**Description:**
Create workflow for production deployment on push to main.
...
- Use same app name extraction as preview workflow
...
**Description:**
Create workflow to cleanup preview deployments on PR close.
...
## Update App Template with Workflows and Documentation
**Priority:** P0
**Estimate:** 1 hour

**Description:**
Integrate all workflows into app-template with comprehensive documentation.
...
# Epic 8: App Migration & Rollout

**Goal:** Roll out GitHub Actions to all existing apps, replacing legacy deploy scripts.
...
**Description:**
Add preview deployment workflows to color-the-map as pilot project. This validates the entire system end-to-end.

**Tasks:**
- Copy workflows from app-template
- Customize for color-the-map specifics (Python tests, linting, etc.)
- Add DEPLOY_API_KEY to GitHub repo secrets
- Create test PR to trigger preview deploy
- Verify preview URL works
- Test the deployed preview app works correctly
- Test merge (production redeploy)
- Test close (cleanup)
- Document any issues

**Acceptance Criteria:**
- Preview deploys on PR open
- Preview URL accessible and app works
- Production redeploys on merge (updates existing deployment)
- Preview cleaned up on close/merge
- Full cycle tested end-to-end
- No issues or all issues documented

**Dependencies:** Update App Template with Workflows and Documentation
...
- Update app-template if workflow changes needed
...
- random-word
- lottery-numbers
- doughughes-net (yes, the homepage uses the system too!)
...
- For each app:
  ...
  - Verify doughughes-net gets catch-all config when deployed to production
  - Document customizations

**Acceptance Criteria:**
- All apps have GitHub Actions workflows
- All tested successfully
- doughughes-net deploys correctly with catch-all handler
- Production deployments update existing apps

**Dependencies:** Add Workflows to home-inventory

---

## Remove Legacy Deploy Scripts from All Apps
**Priority:** P1
**Estimate:** 1 hour

**Description:**
Remove deploy.sh and deploy-to-prod.sh scripts from all apps now that GitHub Actions handles all deployment.
...
**Tasks:**
- For each app:
  - Delete `deploy.sh` from repo
  - Delete `deploy-to-prod.sh` from repo
  - Commit changes with message explaining GitHub Actions replacement
  - Push to trigger production redeploy via GitHub Actions
- Verify all production apps redeploy successfully
- Remove old Caddy configs from server `~/apps/*/caddy*.conf` (no longer used)

**Acceptance Criteria:**
- All legacy deploy scripts removed from app repos
- All apps successfully redeployed via GitHub Actions
- Old server Caddy configs removed
...
**Description:**
Update all infrastructure documentation to reflect the final minimal state.
...
**Tasks:**
- Document infrastructure repo final state:
  - Just Caddy imports and docs
  - No app code
  - No authentication
  - deploy.sh only deploys Caddy config
- Update Caddyfile documentation to show it imports from ~/.config/caddy/
- Add preview deployment system overview
- Update architecture diagrams:
  - Show deploy service
  - Show GitHub Actions flow
  - Remove authentication
  - Show generated configs in ~/.config/
- Explain that apps are deployed via GitHub Actions only
- Document doughughes-net's special catch-all handling
- Document showOnDoughHughesNet field in app.json
- Link to preview deployment vision docs
- Remove references to per-app deploy scripts

**Acceptance Criteria:**
- Infrastructure repo purpose is crystal clear (minimal glue layer)
...
**Dependencies:** Remove Legacy Deploy Scripts from All Apps

---

## Add Deploy Service Monitoring Dashboard (Optional)
**Priority:** P2
**Estimate:** 3 hours
...
**Tasks:**
- Add web UI to deploy service (simple HTML page at /)
- Show active deployments table (production and preview)
- Display port usage (next available port, allocations)
- Add activity log (recent deployments)
- Add link from doughughes-net homepage if desired

**Acceptance Criteria:**
- Dashboard accessible at deploy.doughughes.net (root path)
- Shows useful info
- Helps with debugging
- Looks decent (doesn't need to be fancy)
...
**Tasks:**
- Add created_at timestamp to deployment tracking (in service files or separate metadata)
- Create scheduled task in deploy service (runs daily)
- Identify previews >30 days old
- Call internal cleanup logic for each
- Log cleanup actions
...
- Epic 4 (Deploy Service Dev): ~13.5 hours (6 tickets)
- Epic 5 (Bootstrap): ~2 hours (1 ticket)
- Epic 6 (Self-Deploying): ~4.5 hours (2 tickets)
- Epic 7 (GitHub Actions Templates): ~3.5 hours (1 ticket)
- Epic 8 (App Rollout): ~9-13 hours (5 tickets)
- Epic 9 (Documentation): ~3-6 hours (3 tickets, 2 optional)

**Critical Path:**
Epic 1 (Foundation) → Epic 2 (Simplification) → Epic 3 (Port Management) → Epic 4 (Deploy Service Dev) → Epic 5 (Bootstrap) → Epic 6 (Self-Deploying) → Epic 7 (Templates) → Epic 8 (Rollout) → Epic 9 (Docs)

**Key Milestone:** Epic 6 complete = deploy service can deploy itself = ready to migrate other apps

**Parallelization Opportunities:**
- After Epic 1: Update template and infrastructure docs can happen in parallel
- During Epic 8: Different apps can be migrated in parallel (after pilot)
- Epic 9: Optional monitoring and cleanup can happen in any order

**Risk Areas:**
- Deploy service testing (Epic 4) - must be thorough
- Self-deployment (Epic 6) - chicken-and-egg problem, critical validation
- Pilot app (Epic 8) - may reveal unforeseen issues
- doughughes-net catch-all handling - special case must work
